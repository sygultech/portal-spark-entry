...
-- Create new function with improved week handling
    ...
    IF NOT p_enable_flexible_timings THEN
        ...
        period_data AS (
            SELECT 
                ...
                p.*
            FROM parsed_days pd
            CROSS JOIN jsonb_array_elements(p_default_periods) AS p
            WHERE (p_is_weekly_mode OR pd.week_num IS NOT NULL) -- Only include entries with valid week numbers in fortnightly mode
        )
        ...
        SELECT DISTINCT ON (v_config_id, (p->>'number')::integer, pd.base_day, pd.week_num)
            v_config_id,
            (p->>'number')::integer,
            (p->>'startTime')::time,
            (p->>'endTime')::time,
            ...
            pd.week_num
        FROM period_data pd
        ORDER BY v_config_id, (p->>'number')::integer, pd.base_day, pd.week_num;
    ELSE
        ...
        SELECT DISTINCT ON (v_config_id, (p->>'number')::integer, pd.base_day, pd.week_num)
            ...
            pd.week_num
        FROM parsed_days pd
        CROSS JOIN jsonb_array_elements(p_day_specific_periods->pd.day_key) AS p
        WHERE (p_is_weekly_mode OR pd.week_num IS NOT NULL) -- Only include entries with valid week numbers in fortnightly mode
        ORDER BY v_config_id, (p->>'number')::integer, pd.base_day, pd.week_num;
    END IF;
