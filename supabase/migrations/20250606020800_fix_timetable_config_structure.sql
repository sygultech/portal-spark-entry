-- Add new columns to timetable_configurations
ALTER TABLE "public"."timetable_configurations"
    ADD COLUMN IF NOT EXISTS "default_periods" jsonb,
    ADD COLUMN IF NOT EXISTS "enable_flexible_timings" boolean DEFAULT false;

-- Drop selected_days column if it exists
DO $$ 
BEGIN
    IF EXISTS (
        SELECT 1 
        ...
        AND table_name = 'timetable_configurations' 
        AND column_name = 'selected_days'
    ) THEN
        ALTER TABLE "public"."timetable_configurations" DROP COLUMN "selected_days";
    END IF;
END $$;
...
BEGIN
    ...
    -- Extract selected days from day_specific_periods if provided
    ...
    -- Then handle default timings for remaining selected days
    ...
    CROSS JOIN (
        ...
        WHERE NOT (day = ANY(v_custom_days))  -- Only process days without custom timings
    ) rd
