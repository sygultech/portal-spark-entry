...
-- Create new function with fixed JSON handling
...
BEGIN
    ...
    IF NOT p_enable_flexible_timings THEN
        ...
        SELECT DISTINCT ON (
            v_config_id, 
            (period_data->>'number')::integer, 
            period_data.base_day, 
            period_data.week_num
        )
            v_config_id,
            (period_data->>'number')::integer,
            (period_data->>'startTime')::time,
            (period_data->>'endTime')::time,
            period_data->>'type',
            period_data->>'label',
            period_data.base_day,
            period_data.week_num
