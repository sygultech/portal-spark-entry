-- Create function to create and confirm student user
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Error in create_and_confirm_student_user: %', SQLERRM;
    RAISE;
END;
$$;
...
-- Add RLS policy to ensure only school admins can create students
CREATE POLICY "School admins can create students"
  ...
  WITH CHECK (
    ...
    (SELECT current_setting('role') = 'authenticated' AND current_setting('request.jwt.claims')::json->>'role' = 'authenticated')
  );

-- Enable RLS on student_details if not already enabled
ALTER TABLE public.student_details ENABLE ROW LEVEL SECURITY;

-- Add RLS policy for student_details
CREATE POLICY "School admins can create student details"
  ...
  WITH CHECK (
    ...
    OR 
    -- Allow the function to create student details
    (SELECT current_setting('role') = 'authenticated' AND current_setting('request.jwt.claims')::json->>'role' = 'authenticated')
  ); 